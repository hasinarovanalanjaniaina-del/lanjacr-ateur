...
const DEFAULT_VECTOR_PORTRAIT = `data:image/svg+xml;utf8,` + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="512" height="512">
  <defs>
  ...
  if (!cachedDefaultBase64) {
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        console.error("[portraits] Failed to read default portrait from disk:", ex);
        cachedDefaultBase64 = DEFAULT_VECTOR_PORTRAIT;
      }
    } else {
      cachedDefaultBase64 = DEFAULT_VECTOR_PORTRAIT;
    }
